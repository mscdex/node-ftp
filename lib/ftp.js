...
var RE_XLISTUNIX = XRegExp.cache('^(?<type>[\\-ld])(?<permission>([\\-r][\\-w][\\-xs]){3})\\s+(?<inodes>\\d+)\\s+(?<owner>\\w+)\\s+(?<group>\\w+)\\s+(?<size>\\d+)\\s+(?<timestamp>((?<month1>\\w{3})\\s+(?<date1>\\d{1,2})\\s+(?<hour>\\d{1,2}):(?<minute>\\d{2}))|((?<month2>\\w{3})\\s+(?<date2>\\d{1,2})\\s+(?<year>\\d{4})))\\s+(?<name>.+)$'),
    RE_XLISTMSDOS = XRegExp.cache('^(?<month>\\d{2})(?:\\-|\\/)(?<date>\\d{2})(?:\\-|\\/)(?<year>\\d{2,4})\\s+(?<hour>\\d{2}):(?<minute>\\d{2})\\s{0,1}(?<ampm>[AaMmPp]{1,2})\\s+(?:(?<size>\\d+)|(?<isdir>\\<DIR\\>))\\s+(?<name>.+)$'),
    RE_XTIMEVAL = XRegExp.cache('^(?<year>\\d{4})(?<month>\\d{2})(?<date>\\d{2})(?<hour>\\d{2})(?<minute>\\d{2})(?<second>\\d+)(?:.\\d+)?$'),
    RE_PASV = /([\d]+),([\d]+),([\d]+),([\d]+),([-\d]+),([-\d]+)/,
    RE_EOL = /\r?\n/g,
    RE_CWD = /"(.+)"(?: |$)/,
    RE_PWD = /^"(.+)"(?: |$)/,
...
FTP.prototype.lastMod = function(path, cb) {
  ...
  this._send('MDTM ' + path, function(err, text, code) {
    if (code === 502) {
      ...
      return cb(err);
    var val = XRegExp.exec(text, RE_XTIMEVAL), ret;
    if (!val)
...
function parseListEntry(line) {
  ...
  if (ret = XRegExp.exec(line, RE_XLISTUNIX)) {
    info = {
      type: ret.type,
      name: undefined,
      target: undefined,
    ...
    if (ret.type === 'l') {
      ...
      info.name = ret.name;
    ret = info;
  } else if (ret = XRegExp.exec(line, RE_XLISTMSDOS)) {
    info = {
      name: ret.name,
      type: (ret.isdir ? 'd' : '-'),
