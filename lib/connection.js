...
FTP.prototype.connect = function(options) {
  ...
  socket.once('close', function(had_err) {
    var curReq = self._curReq;
    ondone();
    if (had_err && self.options.reconnect > 0) {
      debug && debug('*****ftp reconnection on error*****');
      --self.options.reconnect;
      self.connect();
      if (curReq) self._send(curReq.cmd, curReq.cb);
    } else {
...
FTP.prototype._pasvConnect = function(ip, port, cb) {
  ...
  socket.once('close', function(had_err) {
    ...
    self._pasvSockets.splice(self._pasvSockets.indexOf(pasvSocket), 1);
