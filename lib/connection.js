...
FTP.prototype.delete = function(path, recursive, cb) {
  if (cb) {
    if (recursive) {
      this.deleteRecursive(path, cb);
    } else {
      this._send('DELE ' + path, cb);
    }
  } else {
    this._send('DELE ' + path, recursive);
  }
};

FTP.prototype.deleteRecursive = function(path, cb) {
  this.list(path, function(err, list) {
    ...
    func = function(err) {
      ...
      var subpath = null;
      if (entry.name[0] === '/') {
        // this will be the case when you call deleteRecursive() and pass the
      ...
      if (entry.type === 'd') {
        this.deleteRecursive(subpath, func);
      } else {
        this.delete(subpath, func);
      }
    }.bind(this);
    func();
  }.bind(this));
